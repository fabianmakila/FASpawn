...
public final class LocationSerializer implements TypeSerializer<Location> {
	...
	public Location deserialize(Type type, ConfigurationNode source) throws SerializationException {
		...
		return new Location(
				world,
				nonVirtualNode(source, X).getDouble(),
				nonVirtualNode(source, Y).getDouble(),
				nonVirtualNode(source, Z).getDouble(),
				nonVirtualNode(source, YAW).getFloat(),
				nonVirtualNode(source, PITCH).getFloat()
		);
	}
